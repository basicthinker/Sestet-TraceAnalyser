...
#define hist_min(ts) ({ \
    ...
    double min = 1024, *pos; \
    for_each_history_limit(pos, fh, fh_len(fh)) { \
      if (*pos < min) min = *pos; \
    } \
    min; })
...
#define threshold(ts_vec) (((ts_vec) + ST_SHORT_INT)->timer * THRE_MULTI)

static inline void update_hist(struct adafs_touch_state ts_vec[],
    int state, double past_int) {
  struct adafs_touch_state *ts = &ts_vec[state];
  fh_update_interval(&ts->int_hist, past_int);
  if (state == ST_SHORT_INT) ts->timer = hist_avg(ts);
  else if (state == ST_LONG_INT) ts->timer = hist_min(ts);
}
